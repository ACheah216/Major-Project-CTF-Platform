...
<head>
    ...
    <title>DNS Lookup - Stage 3</title>
</head>
<body>
    <h1>DNS Lookup - Stage 3</h1>
    ...
    if (!empty($_POST['ip'])) {
        ...
        if (sizeof($octet) == 4) {
            ...
            if ((is_numeric($octet[0])) && (is_numeric($octet[1])) && (is_numeric($octet[2])) && (is_numeric($octet[3]))) {
                ...
                if (strpos($octet[3], ' ') !== false) {
                    echo "<script>alert('Invalid IP Address format provided'); window.location.href = 'rceS3.php';</script>";
                } else {
                    ...
                    if (isset($octetChars[0]) && is_numeric($octetChars[0]) && $blockedChar) {
                        ...
                        $flag = "YCEP2024-R3M0T3C0MM4ND3X3CUT10N(ST4G333RHT)";
                        echo "<script>alert('Congratulations! This is your flag: $flag'); window.location.href = 'rceS3.php';</script>";
                        unset($target, $octet, $cmd);
                    } else {
        ...
        // There is no ip
        echo "<script>alert('Please enter an IP Address and try again'); window.location.href = 'rceS3.php';</script>";
    }
}
?>

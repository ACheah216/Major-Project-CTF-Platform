...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (checkregex($regex_arr, $userID)){
        ...
        renderunknownerror($hint_str, $userID);
        exit;
    }else{
        // check that only 4 quotes are used 
        echo '<!-- moving on to next check... -->';
        $regex_arr = array('/\'|"/');
        if (checkregex($regex_arr, $userID, 4)){
            $hint_str = 'You might have entered a blacklisted symbol. Try again.';
            renderunknownerror($hint_str, $userID);
            exit;
        }else{
            echo '<!-- moving on with program... -->';
        }
    }
...
function renderunknownerror($hint_str, $userID){ 
    ?>
    <span class="error">
        Error: Unexpected input entered. Please try again.
    </span>
    <br>You have entered: 
    <span class="mono"><?= htmlspecialchars($userID) ?></span>
    <script>
        console.log("Did not expect to see this? <?= $hint_str?>"); 
    </script>
    <?php

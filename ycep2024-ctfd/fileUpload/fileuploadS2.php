...
<head>
    ...
    <title>Ask a Question - Stage 2</title>
</head>
<body>
    <h1>Ask a Question - Stage 2</h1>
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            if (count($filenameParts) == 2) {
                ...
                if (isset($filenameParts[1]) && in_array($filenameParts[1], $allowedExtensions)) {
                    ...
                    echo "<script>alert('Question posted successfully'); window.location.href = 'fileuploadS2.php';</script>"; 

                } else {
                    // File ext is not allowed
                    unset($question, $uploadedFile, $uploadedFileName, $uploadedFileTmp, $uploadedFileSize, $destination, $filenameParts);
                    echo "<script>alert('Error posting question. Please check your file extension and try again'); window.location.href = 'fileuploadS2.php';</script>";
                }
                ...
                // Check from the second element onwards for disallowed extensions (Cuz the first element is file name)
                for ($i = 1; $i < count($filenameParts); $i++) {
                    if (in_array($filenameParts[$i], $disallowedExtensions) && in_array(end($filenameParts), $allowedExtensions)) {
                        // Found disallowed extension, echo fail
                        $flag = "YCEP2024-F1L3UPLO4DVULN3RAB1L1TY(ST4G3TVVO2)";
                        echo "<script>alert('Congratulations! This is your flag: $flag'); window.location.href = 'fileuploadS2.php';</script>";
            ...
            unset($uploadedFileTmp, $uploadedFileSize, $filenameParts); // Unset unnecessary variables
            echo "<script>alert('Question posted successfully'); window.location.href = 'fileuploadS2.php';</script>";
        }
    } else {
        // There is no text
        echo "<script>alert('Please fill in the question you have and try again'); window.location.href = 'fileuploadS2.php';</script>";
    }
}
?>
